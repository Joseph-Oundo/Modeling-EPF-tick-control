...
#Source files for generating figure 6
...
run.model.by.treatment.interval.and.coverage <- function(coverage_level, treatment.interval, time, initial.state, param){
  ...
  cattle.tick.model.output <- as.data.frame(cattle.tick.model.output) # Convert model output to a data frame
  
  return(cattle.tick.model.output) # Return results
...
cattle.tick.model <- function(time, state, parameters, input){  
  with(as.list(c(state, parameters)),{
    ...
    dLF_US = LQ_US*alpha_L - LF_US*mu_LF - 
      LF_US*(H_U/H + H_T/H * (1-p_T)) * (H_S/H + H_I/H*(1-p_LI) + H_C/H*(1-p_LC)) * (1/d_L) -
      LF_US*(H_T/H * p_T) * (H_S/H + H_I/H*(1-p_LI) + H_C/H*(1-p_LC)) * 1/(d_L + tau_L) - 
      LF_US*(H_U/H + H_T/H * (1-p_T)) * ((H_I/H * p_LI) + (H_C/H * p_LC)) * (1/d_L) -
      LF_US*(H_T/H * p_T) * ((H_I/H * p_LI) + (H_C/H * p_LC)) * 1/(d_L + tau_L)
    
    dLD_US = LF_US*(H_U/H + H_T/H * (1-p_T)) * (H_S/H + H_I/H*(1-p_LI) + H_C/H*(1-p_LC)) * (1/d_L) - 
      LD_US*mu_LD - LD_US*k_L
    
    dLD_TS = LF_US*(H_T/H * p_T) * (H_S/H + H_I/H*(1-p_LI) + H_C/H*(1-p_LC)) * 1/(d_L + tau_L) - 
      LD_TS*(mu_LD + mu_LDF) - LD_TS*k_L
    
    dLD_UI = LF_US*(H_U/H + H_T/H * (1-p_T)) * ((H_I/H * p_LI) + (H_C/H * p_LC)) * (1/d_L) - 
      LD_UI*mu_LD - LD_UI*k_L
    
    dLD_TI = LF_US*(H_T/H * p_T) * ((H_I/H * p_LI) + (H_C/H * p_LC)) * 1/(d_L + tau_L) - 
    ...
    dNF_US = NQ_US*alpha_N - NF_US*mu_NF - 
      NF_US*(H_U/H + H_T/H * (1-p_T)) * (H_S/H + H_I/H * (1-p_NI) + H_C/H * (1-p_NC)) * (1/d_N) -
      NF_US*(H_T/H * p_T) * (H_S/H + H_I/H * (1-p_NI) + H_C/H * (1-p_NC)) * 1/(d_N + tau_N) - 
      NF_US*(H_U/H + H_T/H * (1-p_T)) * ((H_I/H * p_NI) + (H_C/H * p_NC)) * (1/d_N) -
      NF_US*(H_T/H * p_T) * ((H_I/H * p_NI) + (H_C/H * p_NC)) * 1/(d_N + tau_N)
    
    dNF_UI = NQ_UI*alpha_N - NF_UI*mu_NF - NF_UI*(H_U/H + H_T/H * (1-p_T)) * (1/d_N) -
      NF_UI*(H_T/H * p_T) * 1/(d_N + tau_N) 
    
    dND_US = NF_US*(H_U/H + H_T/H * (1-p_T)) * (H_S/H + H_I/H * (1-p_NI) + H_C/H * (1-p_NC)) * (1/d_N) -
      ND_US*mu_ND - ND_US*k_N
    
    dND_TS = NF_US*(H_T/H * p_T) * (H_S/H + H_I/H * (1-p_NI) + H_C/H * (1-p_NC)) * 1/(d_N + tau_N) -
      ND_TS*(mu_ND + mu_NDF) - ND_TS*k_N
    
    dND_UI = NF_US*(H_U/H + H_T/H * (1-p_T)) * ((H_I/H * p_NI) + (H_C/H * p_NC)) * (1/d_N) +
      NF_UI*(H_U/H + H_T/H * (1-p_T)) * (1/d_N) - ND_UI*mu_ND - ND_UI*k_N
    
    dND_TI = NF_US*(H_T/H * p_T) * ((H_I/H * p_NI) + (H_C/H * p_NC)) * 1/(d_N + tau_N) +
      NF_UI*(H_T/H * p_T) * 1/(d_N + tau_N) - ND_TI*(mu_ND + mu_NDF) - ND_TI*k_N
    ...
    dAF_US = AQ_US*alpha_A - AF_US*mu_AF - 
      AF_US*(H_U/H + H_T/H * (1-p_T)) * (1/d_A) -
      AF_US*(H_T/H * p_T) * 1/(d_A + tau_A)
    
    dAF_UI = AQ_UI*alpha_A - AF_UI*mu_AF - AF_UI*(H_U/H + H_T/H * (1-p_T)) * (1/d_A) -
      AF_UI*(H_T/H * p_T) * 1/(d_A + tau_A)  
    
    dAD_U = AF_US*(H_U/H + H_T/H * (1-p_T)) * (1/d_A) - AD_U*mu_AD - AD_U*(1/k_o) + 
      AF_UI*(H_U/H + H_T/H * (1-p_T)) * (1/d_A) 
    
    dAD_T = AF_US*(H_T/H * p_T) * 1/(d_A + tau_A) - AD_T*(mu_AD + mu_ADF) - 
      AD_T*1/(k_o + k_tau) + AF_UI*(H_T/H * p_T) * 1/(d_A + tau_A) 
